...
bool evHarrisANMSDetector::isFeature(const dvs_msgs::Event &e, int & F1, int & F2)
{
  ...
  for (int i=-nms_size_;i<=nms_size_;++i)
  {
    for (int j=-nms_size_;j<nms_size_;++j)
    {
        if (i==0 && j==0)
            continue;
        // exp(-(double(i*i+j*j))/(2.0*2.0*2.0))*
  ...
  // smooth
  //ssae_[pol](e.x,e.y)=(ssae_[pol](e.x-1,e.y-1)+ssae_[pol](e.x-1,e.y)+ssae_[pol](e.x,e.y+1)
  //                    +ssae_[pol](e.x,e.y-1)+ssae_[pol](e.x,e.y)+ssae_[pol](e.x,e.y+1)
  //                    +ssae_[pol](e.x+1,e.y-1)+ssae_[pol](e.x+1,e.y)+ssae_[pol](e.x+1,e.y+1))/9.0;
  return true;
  // End : Code 3 for ANMS
}
