...
bool FIVCompare(const FIndexedValue& Ihs, const FIndexedValue& Rhs){
        return Ihs.value < Rhs.value;
}

double Flambda_sae(double tival,double tau_smooth)
{
    if (tival < 4*tau_smooth)
    {
        //return 1-(tival/4*tau_smooth)*(1-p_down);
...
FAHarrisANMSDetector::FAHarrisANMSDetector(bool connect) :
Detector(connect),
...
FAHarrisANMSDetector::~FAHarrisANMSDetector() {
}

bool FAHarrisANMSDetector::isFiltered(const dvs_msgs::Event &e)
{
...
bool FAHarrisANMSDetector::isFeature(const dvs_msgs::Event &e, int & F1, int & F2)
{
    ...
    // First filtered by the SAE*
    if (isFiltered(e))
    {
        return false;
    }
    
    //Second filtered by the Arc
    if (!isCornerCandidate(e))
    {
        return false;
    }
    
    //Third filtered by the harris
    double score = isCornerCandidateRefined(e);
    
    double et = e.ts.toSec();
    bool ep = e.polarity;
    const int pol = ep ? 1 : 0;
    score = score < harris_threshold_ ? 0.0 : score;
    ...
    //score > harris_threshold
    F1 = 1;
    F2=1;
    
    // start Tau

    std::vector<FIndexedValue> local_sae;
    ...
    std::partial_sort(local_sae.begin(),local_sae.begin()+5,local_sae.end(),FIVCompare);
    ...
    for (int i=-nms_size_;i<=nms_size_;++i)
    {
      for (int j=-nms_size_;j<nms_size_;++j)
      {
          ...
          if (Flambda_sae(et-sae_[pol](e.x+i,e.y+j),tau_aver)*ssae_[pol](e.x+i,e.y+j) > score)
          {
...
bool FAHarrisANMSDetector::addNewEventToTimeSurface(const dvs_msgs::Event &e)
{
    double et = e.ts.toSec();
    int ex = e.x;
    int ey = e.y;
...
bool FAHarrisANMSDetector::isCornerWithoutFilter(const dvs_msgs::Event &e)
{
    // Return if too close to the border
    if (e.x < window_size_ or e.x >= sensor_width_ - window_size_ or e.y < window_size_ or e.y >= sensor_height_ - window_size_)
    {
...
bool FAHarrisANMSDetector::isCornerCandidate(const dvs_msgs::Event &e) 
{
    int ex = e.x;
    int ey = e.y;
...
double FAHarrisANMSDetector::isCornerCandidateRefined(const dvs_msgs::Event &e)
{
...
double FAHarrisANMSDetector::getHarrisScore(int img_x, int img_y, bool polarity)
{
...
bool FAHarrisANMSDetector::checkPatch(const dvs_msgs::Event &e)
{
    patch_ = Eigen::MatrixXi::Constant(9, 9, 0);
...
Eigen::MatrixXi FAHarrisANMSDetector::getPatch()
{
    /*
...
int FAHarrisANMSDetector::factorial(int n) const
{
    if (n > 1)
    {
...
int FAHarrisANMSDetector::pasc(int k, int n) const
{
    if (k >= 0 && k <= n)
    {
