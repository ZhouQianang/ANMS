...
bool ArcFASTANMSDetector::isFeature(const dvs_msgs::Event &e, int & F1, int & F2)
{
  ...
  // Update Surface of Active Events
    ...
    double score1 = 0., score2 = 0., score = 0.;
    ...
    if(F1)
    {
        ...
        for (int i=-tau_size_;i<=tau_size_;++i)
        {
            for (int j=-tau_size_;j<tau_size_;++j)
            {
                ...
                if (sae_[pol](ex+i,ey+j)>maxt[0])
                {
                    maxt[4]=maxt[3];
                    maxt[3]=maxt[2];
                    maxt[2]=maxt[1];
                    maxt[1]=maxt[0];
                    maxt[0] = sae_[pol](ex+i,ey+j);
                }
            }
        }
        ...
        tau = 20.0*timediff / double(timenum);
        ...
        for (int i=-nms_size_;i<=nms_size_;++i)
        {
            for (int j=-nms_size_;j<nms_size_;++j)
            {
                ...
                if (exp(-(et-sae_[pol](ex+i,ey+j))/tau)*ssae_[pol](ex+i,ey+j) > score)
                {
                    nnms ++;
                    if (nnms > 2 )
                        ssae_[pol](ex,ey) = 0.0;
